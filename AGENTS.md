...
- `src/core/db/`: connection, unit of work, pagination, ordering, and generated SeaORM entities.
- `src/core/layer/`: Axum/Tower middleware such as auth, CORS, lang, trace, and transaction layers.
- `src/core/task/`: task handlers and task types.

Runtime configuration and command entry points live in `src/config/`. Binaries are declared in `Cargo.toml`: the app binary is `my-axum`, with additional `runbook` and `worker` binaries under `src/config/cmd/bin/`. Integration and module tests live under `tests/`, mirroring runtime areas. Benchmarks are kept in `benchmark/`.
...
Generated SeaORM entities in `src/core/db/entity/` should not be hand-edited unless the change is intentionally generated or reviewed as generated code. Run `make lint` before opening a PR; formatting is enforced through `cargo fmt`, `cargo clippy`, and the local pre-commit hook.
...
Coverage intentionally excludes entry points, generated entities, and broker-heavy infrastructure in `Makefile` via `COV_IGNORE`. Do not remove those exclusions unless you are also making those areas testable without external services.
...
If you regenerate SeaORM entities with `make db-generate`, review the diff carefully before committing generated files in `src/core/db/entity/`. Migration files should be deterministic, reversible when practical, and named for the schema behavior they introduce.
