...
.PHONY: prod
prod:
	APP_HOST=0.0.0.0 RUST_LOG=info,sqlx::query=warn,rdkafka=warn cargo run --release --bin my-axum
...
test-cov: test-db-cleanup
	cargo llvm-cov --summary-only --fail-under-functions 80 --fail-under-lines 90 --fail-under-regions 80 $(COVERAGE_IGNORE)

.PHONY: test-cov-report
test-cov-report: test-db-cleanup
	cargo llvm-cov --html --no-cfg-coverage --open $(COVERAGE_IGNORE)
...
db-wait:
	...
	until timeout 1 bash -c "</dev/tcp/$$DB_HOST/$$DB_PORT" > /dev/null 2>&1; do \
