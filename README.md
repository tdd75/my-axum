...
│   ├── config/                    # Stable config, app bootstrap, binary entrypoints
│   ├── core/                      # Routing, DB, middleware, async tasks, runbooks, templates
...
| `ALLOWED_ORIGINS` | `*` | CORS origins, also used to derive MCP Host validation |
| `WORKER_POOL_SIZE` | `10` | Concurrent worker task slots |
| `PAGE_SIZE_LIMIT` | unset | Optional maximum `page_size` accepted by paginated APIs |
...
The app exposes an MCP server at `/mcp` using the official Rust MCP SDK and the Streamable HTTP transport. The MCP layer is an adapter over the existing HTTP API: tools and resources call the normal `/api/v1/...` endpoints and forward authentication headers, so existing API middleware, permission checks, locale handling, and response shapes remain the source of truth.
...
The server runs in stateful Streamable HTTP mode. Call `initialize` first, read the `Mcp-Session-Id` response header, and include that header on later MCP requests in the same session.

Initialize:

```bash
curl -i -N -X POST http://localhost:8000/mcp \
...
| Tool | Backing API | Access |
| --- | --- | --- |
| `get_current_user_profile` | `GET /api/v1/user/profile/` | Authenticated user |
| `search_users` | `GET /api/v1/user/` | Admin |
| `get_user` | `GET /api/v1/user/{id}/` | Admin |

Available resources:

| Resource URI | Backing API | Access |
| --- | --- | --- |
| `my-axum://user/profile` | `GET /api/v1/user/profile/` | Authenticated user |
| `my-axum://users` | `GET /api/v1/user/?page=1&page_size=50&order_by=+id` | Admin |

Responses are returned as `text/event-stream` messages. The server disables empty SSE priming events, so clients should see only data-bearing `message` events for request/response calls.

MCP Host validation follows `ALLOWED_ORIGINS`. Exact origins add their host to the MCP allowlist, while `*` and wildcard origins relax MCP Host validation in the same spirit as the CORS setting. For a public reverse proxy, configure `ALLOWED_ORIGINS` with the browser origin that should be allowed:

```bash
ALLOWED_ORIGINS=https://app.example.com
```

MCP extracts `app.example.com` from that origin and allows it as a Host header for `/mcp`.
