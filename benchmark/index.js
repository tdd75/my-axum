...
function getProfileFlow() {
    ...
    const res = http.get(
        ...
        requestParams(session, 'user_profile', 'read'),
    );
    ...
    check(res, {
        ...
        'user/profile/ returns user id': () => body && Number(body.id) > 0,
    });
...
function updateProfileFlow() {
    ...
    check(res, {
        'user/profile/ patch returns 200': (r) => r.status === 200,
        'user/profile/ patch echoes first_name': () =>
...
function hydrateSessionProfile(session) {
    const res = http.get(
        `${BASE_URL}/api/v1/user/profile/`,
        requestParams(session, 'user_profile_bootstrap', 'auth'),
    );
    const body = safeJson(res);
