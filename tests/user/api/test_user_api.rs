...
mod get_user_tests {
    ...
    use crate::{setup::app::TestApp, setup::fixture::login_admin_user};
...
mod delete_user_tests {
    ...
    async fn test_delete_user_success() {
        ...
        let new_context = Context::builder(Arc::new(new_txn)).build();
...
mod search_user_tests {
    ...
    async fn test_search_user_no_filters() {
        ...
        let access_token = test_app
            ...
            .transaction::<_, String, DbErr>(|txn| {
                Box::pin(async move {
                    let mut context = Context::builder(Arc::new(txn.begin().await?)).build();
                    let (access_token, _user) = login_admin_user(&mut context).await;
    ...
    #[tokio::test]
    async fn test_search_user_applies_configured_page_size_limit() {
        let test_app = TestApp::spawn_app_with_page_size_limit(2).await;
        let client = reqwest::Client::new();
        ...
        let access_token = test_app
            ...
            .transaction::<_, String, DbErr>(|txn| {
                Box::pin(async move {
                    ...
                    for i in 1..=5 {
                        let dto = UserCreateDTO {
                            email: format!("page_limit_{}@example.com", i),
                            password: "password123@".to_string(),
                            first_name: Some(format!("Limit{}", i)),
                            last_name: Some("User".to_string()),
                            phone: None,
                        };
        ...
        let response = client
            .get(format!(
                "http://{}/api/v1/user/?email=page_limit_&page_size=10&order_by=+id",
                &test_app.base_url
            ))
            .bearer_auth(access_token)
            .send()
    ...
    #[tokio::test]
    async fn test_search_user_applies_configured_page_size_limit_without_query() {
        let test_app = TestApp::spawn_app_with_page_size_limit(2).await;
        let client = reqwest::Client::new();
        ...
        let access_token = test_app
            ...
            .transaction::<_, String, DbErr>(|txn| {
                Box::pin(async move {
                    ...
                    for i in 1..=5 {
                        let dto = UserCreateDTO {
                            email: format!("page_limit_no_query_{}@example.com", i),
                            password: "password123@".to_string(),
                            first_name: Some(format!("NoQueryLimit{}", i)),
                            last_name: Some("User".to_string()),
                            phone: None,
                        };
                        create_user_use_case::execute(&context, dto).await.unwrap();
        ...
        let items = result.get("items").unwrap().as_array().unwrap();
        assert_eq!(items.len(), 2);
        assert_eq!(result.get("count").unwrap().as_u64().unwrap(), 6);
    }

    #[tokio::test]
    async fn test_search_user_forbidden_for_normal_user() {
        // Arrange
        let test_app = TestApp::spawn_app().await;
        let client = reqwest::Client::new();
...
mod upload_avatar_tests {
    ...
    #[tokio::test]
    async fn test_upload_avatar_allows_normal_user_without_user_id() {
        let test_app = TestApp::spawn_app().await;
        let client = Client::new();
        ...
        let response = client
            .post(format!(
                "http://{}/api/v1/user/upload-avatar/",
                &test_app.base_url
            ))
            .bearer_auth(access_token)
            .json(&json!({
        ...
        assert_eq!(
            ...
            Some("Internal Server Error: Producer not available")
        );
    }
}
