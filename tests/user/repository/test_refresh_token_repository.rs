...
use my_axum::user::entity::{refresh_token, user};
...
async fn test_search_by_user_id() -> Result<(), DbErr> {
    ...
    let (results, total_count) = refresh_token_repository::search(&context, &params).await?;
...
async fn test_search_expired_tokens() -> Result<(), DbErr> {
    ...
    let (expired_results, expired_total_count) =
        refresh_token_repository::search(&context, &params).await?;
    assert_eq!(expired_results.len(), 2);
    assert_eq!(expired_total_count, 2);
    ...
    let (valid_results, valid_total_count) =
        refresh_token_repository::search(&context, &params).await?;
    assert_eq!(valid_results.len(), 1);
    assert_eq!(valid_total_count, 1);
...
async fn test_delete_by_tokens_batch() -> Result<(), DbErr> {
    ...
    let (before_delete, before_delete_total_count) =
        refresh_token_repository::search(&context, &params).await?;
    assert_eq!(before_delete.len(), 5);
    assert_eq!(before_delete_total_count, 5);

    // Delete tokens in batch
    refresh_token_repository::delete_by_tokens(&context, &token_values).await?;

    // Verify all tokens are deleted
    let (after_delete, after_delete_total_count) =
        refresh_token_repository::search(&context, &params).await?;
    assert_eq!(after_delete.len(), 0);
    assert_eq!(after_delete_total_count, 0);
...
async fn test_search_with_pagination() -> Result<(), DbErr> {
    ...
    let (page1, page1_total_count) = refresh_token_repository::search(&context, &params).await?;
    assert_eq!(page1.len(), 5);
    assert_eq!(page1_total_count, 10);
    ...
    let (page2, page2_total_count) = refresh_token_repository::search(&context, &params).await?;
    assert_eq!(page2.len(), 5);
    assert_eq!(page2_total_count, 10);
