...
async fn test_search_no_filters() -> Result<(), DbErr> {
    ...
    let (result, _total_count) = user_repository::search(
        &context,
        &UserSearchParams {
            ..Default::default()
        },
...
async fn test_search_pagination() -> Result<(), DbErr> {
    ...
    let (page1_result, page1_count) = user_repository::search(
        &context,
        &UserSearchParams {
    ...
    assert_eq!(page1_count, 15);

    // Test second page (page 2) with page size 5
    let (page2_result, page2_count) = user_repository::search(
        &context,
        &UserSearchParams {
            email: Some("pagination_test"),
            page: Some(2),
    ...
    assert_eq!(page2_count, 15);

    // Test third page (page 3) with page size 5
    let (page3_result, page3_count) = user_repository::search(
        &context,
        &UserSearchParams {
            email: Some("pagination_test"),
            page: Some(3),
    ...
    assert_eq!(page3_count, 15);

    // Test fourth page (page 4) with page size 5 - should have no results or fewer
    let (page4_result, page4_count) = user_repository::search(
        &context,
        &UserSearchParams {
            email: Some("pagination_test"),
            page: Some(4),
