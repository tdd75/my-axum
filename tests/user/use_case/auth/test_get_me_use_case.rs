#[cfg(test)]
mod get_me_use_case_tests {
    use crate::setup::app::TestApp;
    use my_axum::{
        core::context::Context,
        user::{
            dto::user_dto::UserCreateDTO,
            use_case::{auth::get_me_use_case, user::create_user_use_case},
        },
    };
    ...
    async fn test_get_me_success() {
        ...
        // Execute get_me
        let result = get_me_use_case::execute(&context_with_user).await;
        ...
        assert!(user_data.created_user.is_none());
        assert!(user_data.updated_user.is_none());
    }

    #[tokio::test]
    async fn test_get_me_with_minimal_user_data() {
        let test_app = TestApp::spawn_app().await;
        let txn = test_app.begin_transaction().await;
        let context = Context {
            txn: &txn,
    ...
    #[tokio::test]
    async fn test_get_me_with_different_users() {
        let test_app = TestApp::spawn_app().await;
        let txn = test_app.begin_transaction().await;
        let context = Context {
            txn: &txn,
    ...
    #[tokio::test]
    async fn test_get_me_preserves_user_model_data() {
        let test_app = TestApp::spawn_app().await;
        let txn = test_app.begin_transaction().await;
        let context = Context {
            txn: &txn,
    ...
    #[tokio::test]
    async fn test_get_me_without_user_context() {
        let test_app = TestApp::spawn_app().await;
        let txn = test_app.begin_transaction().await;
        let context = Context {
            txn: &txn,
        ...
        let result = get_me_use_case::execute(&context).await;
    ...
    #[tokio::test]
    async fn test_get_me_error_handling() {
        let test_app = TestApp::spawn_app().await;
        let txn = test_app.begin_transaction().await;
    ...
    #[tokio::test]
    async fn test_get_me_context_user_cloning() {
        let test_app = TestApp::spawn_app().await;
        let txn = test_app.begin_transaction().await;
        let context = Context {
            txn: &txn,
        ...
        let result1 = get_me_use_case::execute(&context_with_user).await;
        let result2 = get_me_use_case::execute(&context_with_user).await;
    ...
    #[tokio::test]
    async fn test_get_me_user_service_integration() {
        let test_app = TestApp::spawn_app().await;
        let txn = test_app.begin_transaction().await;
        let context = Context {
            txn: &txn,
